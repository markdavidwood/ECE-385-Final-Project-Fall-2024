...
/*
 * Basically, the xspi.c file is a general set of functions that allow for the MicroBlaze to communicate with
 * any device that follows the SPI protocol. The MAX3421E.c file uses portions of those functions to be able to
 * communicate specifically with the MAX3421E chip on the Urbana Board. The HID.c and transfer.c functions
 * exist as other underlying parts of the device driver to allow full functionality with the USB keyboard, but
 * their purpose is not important. The important part here is understanding the underlying connection between
 * the MicroBlaze and the USB keyboard, which happens along many different files/ideas, such as the MAX3421E, spi_usb,
 * and the MISO/MOSI lines.
 */
...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    // Prepare the buffers for SPI transfer
    u8 SendBuffer[2] = { (reg | 0x02), val };  // Write command
    u8 RecvBuffer[2] = { 0x00, 0x00 };         // Unused in this case

    // Perform the transfer
    if (XSpi_Transfer(&SpiInstance, SendBuffer, RecvBuffer, 2) != XST_SUCCESS) {
        xil_printf("Error in MAXreg_wr: XSpi_Transfer failed\n");
    }
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    // Create the send buffer with reg + data
    u8 SendBuffer[nbytes + 1];
    SendBuffer[0] = reg | 0x02;  // Write command
    for (BYTE i = 0; i < nbytes; i++) {
        SendBuffer[i + 1] = data[i];
    }

    // No need to receive anything, but we pass a dummy buffer
    u8 RecvBuffer[nbytes + 1];

    // Perform the transfer
    if (XSpi_Transfer(&SpiInstance, SendBuffer, RecvBuffer, nbytes + 1) != XST_SUCCESS) {
        xil_printf("Error in MAXbytes_wr: XSpi_Transfer failed\n");
    }

    XSpi_SetSlaveSelect(&SpiInstance, 0x00);  // Deselect MAX3421E

    return data + nbytes;  // Return pointer to the end of the written data
}
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    // Create the SendBuffer
    u8 SendBuffer[2] = {reg, 0x00}; // Clear the write bit for read command
    u8 RecvBuffer[2] = {0x00, 0x00};
	//write reg via SPI
	//read val via SPI
	if(XSpi_Transfer(&SpiInstance, SendBuffer, RecvBuffer, 2) != XST_SUCCESS){
		xil_printf("Error in MAXreg_rd function, failed to complete XSpi_Transfer");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    // Create the send buffer with the register address
    u8 SendBuffer[nbytes + 1];
    SendBuffer[0] = reg;  // Read command
    for (BYTE i = 1; i <= nbytes; i++) {
        SendBuffer[i] = 0x00;  // Dummy bytes for receiving data
    }
    ...
    // Perform the transfer
    if (XSpi_Transfer(&SpiInstance, SendBuffer, RecvBuffer, nbytes + 1) != XST_SUCCESS) {
        xil_printf("Error in MAXbytes_rd: XSpi_Transfer failed\n");
    }
    ...
    return data + nbytes;  // Return pointer to the end of the read data
}
